...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent2", type=str, default="My_agent", help='agent 2 name, only name, not path')


    parser.add_argument("--round", type=int, default=10, help='play rounds')
    parser.add_argument("--fps", type=float, default=500, help='display fps')
    ...
    agent2_path = 'agent/' + args.agent2 + '/super_agent4.py'
    ...
    agent2_import_path = 'agent.' + args.agent2 + '.super_agent4'
    ...
    if not os.path.exists(agent1_path):
        print('Error: agent1 file does not exist!')
        exit(1)
    if not os.path.exists(agent2_path):
        print('Error: agent2 file does not exist!')
        exit(1)
    # delay calc
    if args.fps == 0:
        step_delay = 0
